...
class Quotation extends ModelTenant
{
    ...
    /**
     * 
     * Obtener direccion completa del cliente
     *
     * @return string
     */
    public function getCustomerFullAddress()
    {
        ...
        if(is_null($address) || $address === '-' || $address === '') return null;

        $department = $this->customer->department->description ?? null;
        $province = $this->customer->province->description ?? null;
        $district = $this->customer->district->description ?? null;

        $department_description = $department ? "{$department} - " : '';
        $province_description = $province ? "{$province} - " : '';
        $district_description = $district ? "{$district}" : '';

        $full_address = trim($address).($department ? ', ' : '');

        return "{$full_address}{$department_description}{$province_description}{$district_description}";
    }
