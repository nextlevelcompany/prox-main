...
trait ReportTrait
{
    ...
    {
        $document_type_id = func_in_array($request, 'document_type_id');
        $establishment_id = func_in_array($request, 'establishment_id');
        $period = func_in_array($request, 'period');
        $date_start = func_in_array($request, 'date_start');
        $date_end = func_in_array($request, 'date_end');
        $month_start = func_in_array($request, 'month_start');
        $month_end = func_in_array($request, 'month_end');
        $person_id = func_in_array($request, 'person_id');
        $type_person = func_in_array($request, 'type_person');

        $seller_id = func_in_array($request, 'seller_id');
        $state_type_id = func_in_array($request, 'state_type_id');
        $purchase_order = func_in_array($request, 'purchase_order');
        $guides = func_in_array($request, 'guides');
        $web_platform = func_in_array($request, 'web_platform_id',0);

        $session_user_id = func_in_array($request, 'session_user_id', false);
    ...
    public function getRecordsCash($request){
        ...
        $document_type_id = func_in_array($request,'document_type_id');
        $user_id = func_in_array($request,'user_id');
