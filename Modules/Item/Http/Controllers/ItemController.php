...
use App\Helpers\PdfUnionHelper;
...
use Modules\Item\Exports\DigemidItemExport;
use Modules\Item\Exports\ItemExport;
use Modules\Item\Exports\ItemExportWp;
use Modules\Item\Exports\ItemExtraDataExport;
...
use Modules\Item\Imports\CatalogImport;
use Modules\Item\Imports\ItemImport;
...
use Modules\Item\Models\ItemImage;
use Modules\Item\Models\ItemLotsGroup;
use Modules\Item\Models\ItemMovement;
use Modules\Item\Models\ItemSupply;
use Modules\Item\Models\ItemTag;
use Modules\Item\Models\ItemUnitType;
use Modules\Item\Models\ItemWarehousePrice;
...
use Modules\Item\Imports\ItemUpdatePriceImport;
...
class ItemController extends Controller
{
    ...
    public function tables()
    {
        ...
        $attribute_types = func_get_table_attribute_types();
        $system_isc_types = func_get_table_system_isc_types();
    ...
    public function store(ItemRequest $request)
    {
        ...
        if ($request->has('date_of_due')) {
            ...
            if (isset($time['date'])) {
                $date = $time['date'];
                if (!empty($date)) {
                    $request->merge(['date_of_due' => \Carbon\Carbon::createFromFormat('Y-m-d H:i:s.u', $date)]);
                }
            }
        }
        $current_lot = null;
        ...
        if ($temp_path) {
            ...
            $extension = strtolower(end($file_name_old_array));
    ...
    public function exportBarCodeFull(Request $request)
    {
        ...
        foreach ($records as $item) {
            ...
            $pdf->AddPage();
            $pdf->WriteHTML($html, HTMLParserMode::HTML_BODY);
            PdfUnionHelper::addFpi($pdfj, $pdf);
        }

        return PdfUnionHelper::ResponseAsFile($pdfj, 'bar_code_full');
    ...
    public function searchItemById($id)
    {
        ...
        $items = SearchItemHelper::getItemsToSupply(null, $id);
        return compact('items');
    }

    public function searchItems(Request $request)
    {
        $items = SearchItemHelper::getItemsToSupply($request);
    ...
    public function item_tables()
    {
        ...
        $price_types = func_get_table_price_types();
        $attribute_types = func_get_table_attribute_types();
