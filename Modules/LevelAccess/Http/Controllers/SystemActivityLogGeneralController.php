...
use Modules\LevelAccess\Http\Resources\SystemActivityLogCollection;
use Carbon\Carbon;
...
class SystemActivityLogGeneralController extends Controller
{
    ...
    {
        return view('tenant.level_access.system_activity_logs.generals.index');
    }
    ...
    public function exportReport($type, Request $request)
    {
        if ($type === 'excel') {
            ...
            $general_format_export->view_name("tenant.level_access.system_activity_logs.reports.general_{$type}")->data($data);
