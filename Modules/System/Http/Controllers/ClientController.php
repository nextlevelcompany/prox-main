...
use Modules\System\Http\Resources\ClientResource;
...
class ClientController extends Controller
{
    ...
    public function charts()
    {
        ...
        foreach ($records as $row) {
            ...
            for ($i = 1; $i <= 12; $i++) {
                ...
                $date_before = Carbon::now()->subYear();
                $year_before = $date_before->format('Y');
                $days_in_month = $date_before->daysInMonth;
                $date_final = Carbon::parse(date('Y') . '-' . $i . '-' . $days_in_month);
                $count_documents[] = [
                    'client' => $row->number,
                    'month' => $i,
    ...
    public function validateWebsite($uuid)
    {
        $exists = Website::query()->where('uuid', $uuid)->first();
