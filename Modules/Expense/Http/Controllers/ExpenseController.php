...
class ExpenseController extends Controller
{
    ...
    {
        return view('tenant.expense.expenses.index');
    }


    public function create($id = null)
    {
        return view('tenant.expense.expenses.form', compact('id'));
    }
    ...
    public function toPrint($external_id, $format = 'a4')
    {
        ...
        $path = Storage::disk('tenant')->path('expense'.DIRECTORY_SEPARATOR.$record->filename, $this->getStorage($record->filename, 'expense'));

        return response()->file($path, $headers);
        // return GeneralPdfHelper::getPreviewTempPdf('expense', $this->getStorage($record->filename, 'expense')); // error tempnam()
    }
