...
use Modules\Catalog\Models\PaymentMethodType as CatPaymentMethodType;
use Modules\Company\Models\Company;
use Modules\Company\Models\Configuration;
use Modules\Dispatch\Models\Dispatch;
use Modules\Document\Http\Requests\DocumentMailRequest;
...
use Modules\Document\Imports\DocumentImportExcelFormat;
use Modules\Document\Imports\DocumentsImport;
use Modules\Document\Imports\DocumentsImportTwoFormat;
...
use Modules\Document\Mail\DocumentMail;
...
use Modules\Item\Http\Resources\SearchItemLotCollection;
...
class DocumentController extends Controller
{
    ...
    public function tables()
    {
        ...
        $operation_types = func_get_table_operation_types();
        ...
        $user_settings = [
            'change_seller' => $user->change_seller
        ];
        $user = $userType;
    ...
    {
        $items = SearchItemHelper::getItemsToDocuments();
        $categories = [];
        $affectation_igv_types = AffectationIgvType::whereActive()->get();
    ...
    public function table($table)
    {
        ...
        if ($table === 'items') {
            ...
            return SearchItemHelper::getItemsToDocuments();
    ...
    {
        $document = Document::applyWithsShow()->findOrFail($documentId);
...
//        $response = $fact->getResponse();
    ...
    public function email(DocumentMailRequest $request)
    {
        $company = Company::active();
        $document = Document::find($request->input('id'));
        $customer_email = $request->input('customer_email');
        $email = $customer_email;
        $mailable = new DocumentMail($company, $document);
        $id = (int)$request->input('id');
        $sendIt = MailHelper::SendMail($email, $mailable, $id, 1);
    ...
    public function data_table()
    {
        ...
        $state_types = StateType::query()->get();
        $document_types = DocumentType::query()->whereIn('id', ['01', '03', '07', '08'])->get();
        $series = Series::query()->whereIn('document_type_id', ['01', '03', '07', '08'])->get();
        $establishments = Establishment::query()->where('id', auth()->user()->establishment_id)->get();
    ...
    /**
     * Buscar anticipos por afectacion
     *
     * @param string $type
     * @return array
     */
    ...
    public function searchItemLots(Request $request)
    {
        ...
        return new SearchItemLotCollection($query->paginate(config('tenant.items_per_page')));
    }
     ...
     * @return array
     */
    public function uploadPayConstancy(Request $request)
    {
