...
use Modules\SaleNote\Http\Resources\SaleNoteResource2;
...
use Modules\Dispatch\Http\Resources\DispatchSaleNoteCollection;
use Modules\Dispatch\Models\Dispatch;
use Modules\Establishment\Models\Establishment;
use Modules\Item\Models\Item;
use Modules\Item\Models\ItemWarehouse;
...
use Modules\SaleNote\Http\Resources\SaleNoteResource;
use Modules\SaleNote\Mail\SaleNoteMail;
use Modules\SaleNote\Models\SaleNote;
use Modules\SaleNote\Models\SaleNoteItem;
use Modules\SaleNote\Models\SaleNoteMigration;
...
use Mpdf\Mpdf;
use Modules\Dispatch\Models\DispatchSaleNote;
...
class SaleNoteController extends Controller
{
    ...
    private function getRecords($request){
        ...
        if($request != null && $request->has('onlySubscription') && (bool)$request->onlySubscription == true){
            $records->whereNotNull('grade')->whereNotNull('section') ;
        }
        // Solo devuelve Suscripciones que tengan relacion en user_rel_subscription_plans.
        if($request != null && $request->has('onlyFullSubscription') && (bool)$request->onlyFullSubscription == true){
            $records->whereNotNull('user_rel_subscription_plan_id')
    ...
    public function tables()
    {
        ...
        $establishments = Establishment::where('id', auth()->user()->establishment_id)->getDataFromRelatedTables()->get();
    ...
    {
        $items = SearchItemHelper::getItemsToSaleNote();
        $categories = [];
    ...
    {
        $record = new SaleNoteResource2(SaleNote::getDataFromRelatedTables()->findOrFail($id));
...
//    }
    ...
    public function table($table)
    {
        switch ($table) {
            ...
            case 'items':
                ...
                return SearchItemHelper::getItemsToSaleNote();
    ...
    public function searchItems(Request $request)
    {
        ...
        */
        $items = SearchItemHelper::getItemsToSaleNote($request);

        /*
        $items = SearchItemHelper::getItemsToSaleNote($request)->transform(function ($row) use ($warehouse_id, $warehouse) {
            $detail = $this->getFullDescription($row, $warehouse);
...
*/
    ...
    {
        return  SearchItemHelper::getItemsToSaleNote(null, $id);
        $establishment_id = auth()->user()->establishment_id;
        $warehouse = Warehouse::where('establishment_id', $establishment_id)->first();
    ...
    public function email(Request $request)
    {
        ...
        $mailable = new SaleNoteMail($company, $record);
        $id = (int) $request->id;
        $sendIt = MailHelper::SendMail($email, $mailable, $id, 2);
    ...
    {
        return SearchItemHelper::TransformToModalSaleNote(Item::whereIn('id', $request->ids)->get());
    }
