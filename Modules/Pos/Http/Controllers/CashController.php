...
class CashController extends Controller
{
    ...
    public function email(Request $request) {
        ...
        $mailable = new CashMail($company, $this->getPdf($request->cash_id));
...
//        $all_documents = collect($all_documents)->sortBy('date_sort')->all();
    ...
    private function getPdf($cash, $format = 'ticket', $mm = null)
    {
        ...
        $view = view('tenant.pos.pos.cash.report_pdf_'.$format, compact('data'));
        if($format === 'simple_a4') {
            $view = view('tenant.pos.pos.cash.report_pdf_'.$format, compact('data'));
        }
        $html = $view->render();
    ...
    {
        $view = view('tenant.pos.pos.cash.reports.report_income_egress_pdf', $data);
        $html = $view->render();
