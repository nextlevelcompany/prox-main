...
use Modules\Purchase\Http\Requests\PurchaseImportRequest;
...
use Modules\Purchase\Http\Resources\PurchaseCollection;
...
class PurchaseController extends Controller
{
    ...
    public function table($table)
    {
        switch ($table) {
            ...
            case 'items':
                return SearchItemHelper::getItemToPurchase();
                return SearchItemHelper::getItemToPurchase()->transform(function ($row) {
                    /*
...
//                return $items;
    ...
    public function item_tables()
    {
        ...
        $items = SearchItemHelper::getItemToPurchase();
        $categories = [];
    ...
    public function searchItemById($id)
    {
        ...
        $items = SearchItemHelper::getItemToPurchase(null, $id);
        $a = null;
        // Solo para que no entre en esta seccion
        if ($a !== null) {
            $items = SearchItemHelper::getNotServiceItemToPurchase(null, $id)->transform(function ($row) {
                /** @var Item $row */
                $full_description = ($row->internal_id) ? $row->internal_id . ' - ' . $row->description : $row->description;
                return [
                    'id' => $row->id,
    ...
    {
        $items = SearchItemHelper::getItemToPurchase($request);
        // Solo para evitar que entre en esta seccion
        $a = null;
        if ($a != null) {
            $items = SearchItemHelper::getItemToPurchase($request)->transform(function ($row) {
                /** @var Item $row */
                $full_description = ($row->internal_id) ? $row->internal_id . ' - ' . $row->description : $row->description;
