...
use Modules\Purchase\Mail\PurchaseOrderMail;
use Mpdf\Mpdf;
...
use Modules\Purchase\Http\Requests\PurchaseOrderRequest;
...
class PurchaseOrderController extends Controller
{
    ...
    {
        return view('tenant.purchase.purchase-orders.index');
    }

    public function create($id = null)
    {
        $sale_opportunity = null;
        return view('tenant.purchase.purchase-orders.form', compact('id', 'sale_opportunity'));
    }

    public function generate($id)
    {
        $purchase_quotation = PurchaseQuotation::getDataFromRelatedTables()->findOrFail($id);

        return view('tenant.purchase.purchase-orders.generate', compact('purchase_quotation'));
    }
    ...
    public function tables()
    {
        ...
        $establishment = Establishment::where('id', auth()->user()->establishment_id)->getDataFromRelatedTables()->first();
        ...
        return compact('suppliers', 'establishment', 'company', 'currency_types',
            'payment_method_types', 'series');
    }


    public function item_tables()
    {
        $items = SearchItemHelper::getItemToPurchaseOrder();
    ...
    public function email(Request $request)
    {
        ...
        $mailable = new  PurchaseOrderMail($record);
        $id = (int)$record->id;
        $sendIt = MailHelper::SendMail($email, $mailable, $id, 5);
        /*
    ...
    {
        $items = SearchItemHelper::getItemToPurchaseOrder(null, $id);
    ...
    {
        $items = SearchItemHelper::getItemToPurchaseOrder($request);
