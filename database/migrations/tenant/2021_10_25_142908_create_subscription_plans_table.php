...
class CreateSubscriptionPlansTable extends Migration
{
    ...
    public function up()
    {
        if(!Schema::hasTable('cat_periods')){
        ...
        if(!Schema::hasTable('item_rel_subscription_plans')){
            Schema::create('item_rel_subscription_plans', function (Blueprint $table) {
                $table->increments('id');
                $table->unsignedInteger('item_id')->default(0)->nullable()->comment('Relacion con items');
                $table->json('item')->nullable();

                $table->unsignedInteger('subscription_plan_id')->default(0)->nullable()->comment('Relacion con planes de suscripcion');
        ...
        if(!Schema::hasTable('user_rel_subscription_plans')){
            Schema::create('user_rel_subscription_plans', function (Blueprint $table) {
                $table->increments('id');
        ...
        $mensual = DB::table('cat_periods')->where('period', 'M')->first();
        if(is_null($mensual)){
            $catPeriods[] = [
                'Id' => count($catPeriods) + 1,
                'period' => 'M',
        ...
        $anual = DB::table('cat_periods')->where('period', 'Y')->first();
        if(is_null($anual)){
        ...
        if(is_null($anual) && is_null($mensual)){
                DB::table('cat_periods')->insert($catPeriods);
        }
            $subscriptionPlans = [];

        $splans = DB::table('subscription_plans')->where('name', 'Matricula Escolar')->first();
        if(is_null($splans)){
            $subscriptionPlans[] = [
                'Id' => count($subscriptionPlans) + 1,
                'cat_period_id' => 1,
            ...
            DB::table('subscription_plans')->insert($subscriptionPlans);
        }
    }
    ...
    public function down()
    {
        ...
        Schema::dropIfExists('item_rel_subscription_plans');
        Schema::dropIfExists('user_rel_subscription_plans');
    }
}
