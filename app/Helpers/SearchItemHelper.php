...
class SearchItemHelper
{
     ...
     */
    public static function getNotServiceItem(Request $request = null, $id = 0, $search_item_by_series = false)
    {

        self::validateRequest($request);
        $search_by_barcode = $request->has('search_by_barcode') && (bool)$request->search_by_barcode;
        $input = self::setInputByRequest($request);
        $item = self::getAllItemBase($request, false, $id, $search_item_by_series);
     ...
     */
    public static function getAllItemBase(Request $request = null, $service = false, $id = 0, $search_item_by_series = false)
    {
        self::validateRequest($request);
//        $search_item_by_series = Configuration::first()->isSearchItemBySeries();
        ...
        $item->with('brand', 'category', 'currency_type', 'sale_affectation_igv_type');
...
//            }
            ...
            $item->where('text_filter', 'like', '%' . str_replace(' ', '%', $input) . '%');
            if ($search_factory_code_items) {
                $item->where('factory_code', 'like', '%' . str_replace(' ', '%', $input) . '%');
            }
     ...
     * @return Item[]|\Illuminate\Database\Eloquent\Builder[]|\Illuminate\Database\Eloquent\Collection|Builder[]|Collection|mixed
     */
    public static function getServiceItem(Request $request = null, $id = 0, $search_item_by_series = false)
    {
        self::validateRequest($request);
        $search_by_barcode = $request->has('search_by_barcode') && (bool)$request->search_by_barcode;
        $input = self::setInputByRequest($request);
        /** @var Item $item */
        $item = self::getAllItemBase($request, true, $id, $search_item_by_series = false);
    ...
    /**
     * Retorna la coleccion de items para Documento y Boleta.
     *  Usado en app/Http/Controllers/Tenant/DocumentController.php::250
     *  Usado en app/Http/Controllers/Tenant/DocumentController.php::370
     *  Usado en modules/Document/Http/Controllers/DocumentController.php::297
     *  Usado en App\Http\Controllers\Controller.php::125
     *
     * @param Request| null $request
     * @param int $id
     *
    ...
    {
        $search_item_by_series = Configuration::query()->first()->isSearchItemBySeries();
        $items_not_services = self::getNotServiceItem($request, $id, $search_item_by_series);
        $items_services = self::getServiceItem($request, $id, $search_item_by_series);
        ...
        $item_not_service = $item_not_service
            ...
            ->take(config('tenant.number_items_at_start'))
            ->get();
...
//                $data->OrWhereJsonContains('attributes', ['value' => $input]);
            ...
            $data->take(config('tenant.number_items_in_search'));
        } else {
            // Inicia con 20 productos, puede añadirse en el env la variable NUMBER_ITEMS
            $data->take(config('tenant.number_items_at_start'));
