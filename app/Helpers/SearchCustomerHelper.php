...
use Modules\FullSubscription\Models\Tenant\FullSubscriptionUserDatum;

class SearchCustomerHelper
{
...
//        $operation_type_id_id = $this->getIdentityDocumentTypeId($request->operation_type_id);
     ...
     */
    public static function getSubscriptionCustomers(Request $request = null, ?int $id = 0)
    {
     ...
     */
    public static function getCustomersToSubscriptionList(Request $request = null, ?int $id = 0, $onlyParent = false)
    {
        $person = Person::query();
        $person->with('addresses');
        $orderColum = 'name';
        ...
        if ($request->has('column') && !empty($request->column) && $request->column == 'discord_channel') {
            ...
            $orderColum = 'id';
            $temp = FullSubscriptionUserDatum::where('discord_channel', 'like', "%$value%")->select('person_id')->distinct()->get()->pluck('person_id');
