...
class EmailSendLog extends ModelTenant
{
    ...
    {
        return $query->where('relation_model', Document::class);
    }

    public function scopeSaleNote($query)
    {
        return $query->where('relation_model', SaleNote::class);
    }

    public function scopeQuotation($query)
    {
        return $query->where('relation_model', Quotation::class);
    }

    public function scopeDispatch($query)
    {
        return $query->where('relation_model', Dispatch::class);
    }

    public function scopePurchase($query)
    {
        return $query->where('relation_model', Purchase::class);
    }

    public function scopeFindRelationId($query, $id = 0)
    {
        return $query->where('relation_id', $id);
    }
