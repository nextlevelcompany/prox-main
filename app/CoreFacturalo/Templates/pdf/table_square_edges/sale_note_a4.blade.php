...
<table class="full-width mt-5">
    ...
    <tr>
        ...
        @if($document->purchase_order)
            <td class="align-top" width="120px">Orden de compra:</td>
    ...
    <tr>
        ...
        <td class="align-top" width="120px">Estado:</td>
        <td>{{$document->total_canceled ? 'CANCELADO' : 'PENDIENTE DE PAGO'}} </td>
    ...
    <tr>
        ...
        <td colspan="3">{{ $document->getFormatDueDate() }}</td>
    </tr>
    @endif
</table>
...
<table class="full-width mt-10 mb-10">
    <thead class="">
    <tr class="bg-grey">
        <th class="border-top-bottom border-left text-center py-2" width="8%">COD.</th>
        <th class="border-top-bottom text-center py-2" width="8%">CANT.</th>
        <th class="border-top-bottom text-left py-2">DESCRIPCIÓN</th>
        <th class="border-top-bottom text-center py-2" width="8%">UNIDAD</th>
        <th class="border-top-bottom text-right py-2" width="12%">P.UNIT</th>
        <th class="border-top-bottom text-right py-2" width="8%">DTO.</th>
        <th class="border-top-bottom border-right text-right py-2 pr-3" width="12%">TOTAL</th>
    </tr>
    </thead>
    <tbody>
    @foreach($document->items as $row)
        <tr>
            <td class="text-center border-left align-top">{{ $row->item->internal_id ?? null }}</td>
            ...
            <td class="border-right text-right align-top pr-3">{{ number_format($row->total, 2) }}</td>
        </tr>
        <tr>
            <td colspan="7" class="border-right border-left border-bottom"></td>
        </tr>
    @endforeach
        @if($document->total_exportation > 0)
            <tr>
                <td colspan="6" class="text-right font-bold">OP. EXPORTACIÓN: {{ $document->currency_type->symbol }}</td>
                <td class="text-right font-bold pr-3 ">{{ number_format($document->total_exportation, 2) }}</td>
            </tr>
        @endif
        @if($document->total_free > 0)
            <tr>
                <td colspan="6" class="text-right font-bold">OP. GRATUITAS: {{ $document->currency_type->symbol }}</td>
                <td class="text-right font-bold pr-3">{{ number_format($document->total_free, 2) }}</td>
            </tr>
        @endif
        @if($document->total_unaffected > 0)
            <tr>
                <td colspan="6" class="text-right font-bold">OP. INAFECTAS: {{ $document->currency_type->symbol }}</td>
                <td class="text-right font-bold pr-3">{{ number_format($document->total_unaffected, 2) }}</td>
            </tr>
        @endif
        @if($document->total_exonerated > 0)
            <tr>
                <td colspan="6" class="text-right font-bold">OP. EXONERADAS: {{ $document->currency_type->symbol }}</td>
                <td class="text-right font-bold pr-3">{{ number_format($document->total_exonerated, 2) }}</td>
            </tr>
        @endif
        @if($document->total_taxed > 0)
             <tr>
                <td colspan="6" class="text-right font-bold">OP. GRAVADAS: {{ $document->currency_type->symbol }}</td>
                <td class="text-right font-bold pr-3 ">{{ number_format($document->total_taxed, 2) }}</td>
            </tr>
        @endif
        @if($document->total_discount > 0)
            <tr>
                ...
                <td class="text-right font-bold pr-3">{{ number_format($document->total_discount, 2) }}</td>
            </tr>
        @endif

        <tr>
            <td colspan="6" class="text-right font-bold">IGV: {{ $document->currency_type->symbol }}</td>
            <td class="text-right font-bold pr-3">{{ number_format($document->total_igv, 2) }}</td>
        </tr>
        ...
        @if($document->total_charge > 0 && $document->charges)
            <tr>
                ...
                <td class="text-right font-bold pr-3">{{ number_format($document->total_charge, 2) }}</td>
            </tr>
        @endif

        <tr>
            <td colspan="6" class="text-right font-bold">TOTAL A PAGAR: {{ $document->currency_type->symbol }}</td>
            <td class="text-right font-bold pr-3">{{ number_format($document->total, 2) }}</td>
        </tr>
        ...
        @if($change_payment < 0)
            <tr>
                ...
                <td class="text-right font-bold pr-3">{{ number_format(abs($change_payment),2, ".", "") }}</td>
            </tr>
        @endif
...
<tr>
    ...
    <strong>PAGOS:</strong> </td></tr>
        ...
        @foreach($payments as $row)
            ...
            @endphp
        @endforeach
        <tr><td class="pt-2 mt-2"><strong>SALDO:</strong> {{ $document->currency_type->symbol }} {{ number_format($document->total - $payment, 2) }}</td>
    </tr>
...
@if ($document->terms_condition)
    ...
    <table class="full-width">
        <tr>
            <td>
                <h6 style="font-size: 12px; font-weight: bold;">Términos y condiciones</h6>
