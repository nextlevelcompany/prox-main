...
class ChangelogGen extends Command
{
    ...
    public function handle()
    {
        ...
        $keys = ['update', 'feature', 'fixed', 'docs'];

        $path = base_path();

        foreach ($keys as $key) {
            $command = 'git log '.$option_from.'..'.$option_to.' --no-merges --date=short --pretty=format:"* %ad : %s" | grep -e '.$key;
            $this->info($command);
            $gitlog = Process::fromShellCommandline($command);
            $gitlog->run();
            if (!$gitlog->isSuccessful()) {
                $this->error('No hay datos para '.$key);
            } else {
                $this->info('Datos encontrados para '.$key);
        ...
        $this->info('ha finalizado el proceso');
    }
}
